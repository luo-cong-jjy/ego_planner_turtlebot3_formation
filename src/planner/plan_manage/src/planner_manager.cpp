...
namespace ego_planner
{
  ...
  namespace
  {
    ...
    Eigen::Vector3d planarizeVector(const Eigen::Vector3d &value)
    {
      Eigen::Vector3d projected = value;
      projected(2) = kPlanarZ;
      return projected;
    }
  }
      ...
      poly_traj::MinJerkOpt &initMJO)
  {
    ...
    const Eigen::Vector3d planar_start_pt = planarizeVector(start_pt);
    const Eigen::Vector3d planar_start_vel = planarizeVector(start_vel);
    const Eigen::Vector3d planar_start_acc = planarizeVector(start_acc);
    const Eigen::Vector3d planar_target_pt = planarizeVector(local_target_pt);
    const Eigen::Vector3d planar_target_vel = planarizeVector(local_target_vel);
    ...
    if (flag_first_call || flag_polyInit) /*** case 1: polynomial initialization ***/
    {
      ...
      headState << planar_start_pt, planar_start_vel, planar_start_acc;
      tailState << planar_target_pt, planar_target_vel, Eigen::Vector3d::Zero();
      ...
      {
        Eigen::Vector3d delta = planar_start_pt - planar_target_pt;
        Eigen::Vector3d horizen_dir(-delta(1), delta(0), 0.0);
        if (horizen_dir.norm() < 1e-6)
        {
          horizen_dir = Eigen::Vector3d(0.0, 1.0, 0.0);
        }
        else
        {
          horizen_dir.normalize();
        }
        innerPs.resize(3, 1);
        innerPs = (planar_start_pt + planar_target_pt) / 2 +
                  (((double)rand()) / RAND_MAX - 0.5) *
                      (planar_start_pt - planar_target_pt).norm() *
                      horizen_dir * 0.8 * (-0.978 / (continous_failures_count_ + 0.989) + 0.989);
        innerPs.row(2).setZero();
      ...
      piece_nums = round((headState.col(0) - tailState.col(0)).head<2>().norm() / pp_.polyTraj_piece_length);
      if (piece_nums < 2)
        piece_nums = 2;
    ...
    else /*** case 2: initialize from previous optimal trajectory ***/
    {
      ...
      int piece_nums = ceil((planar_start_pt - planar_target_pt).head<2>().norm() / pp_.polyTraj_piece_length);
      if (piece_nums < 2)
        piece_nums = 2;
