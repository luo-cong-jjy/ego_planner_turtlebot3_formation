...
{
	Eigen::Vector3d coord = ((index - CENTER_IDX_).cast<double>() * step_size_) + center_;
	coord(2) = center_(2);
	return coord;
};
...
{
	Eigen::Vector3d planar_pt = pt;
	planar_pt(2) = center_(2);
	idx = ((planar_pt - center_) * inv_step_size_ + Eigen::Vector3d(0.5, 0.5, 0.5)).cast<int>() + CENTER_IDX_;
	idx(2) = CENTER_IDX_(2);
