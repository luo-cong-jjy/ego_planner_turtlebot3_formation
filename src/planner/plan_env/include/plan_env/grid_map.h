...
inline int GridMap::getOccupancy(Eigen::Vector3d pos)
{
  ...
  if (mp_.use_2d_navigation_)
  {
    for (int z = 0; z < mp_.map_voxel_num_(2); ++z)
    {
      Eigen::Vector3i query_id(id(0), id(1), z);
      if (md_.occupancy_buffer_[toAddress(query_id)] > mp_.min_occupancy_log_)
      {
        return 1;
      }
    }
...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos)
{
  ...
  if (mp_.use_2d_navigation_)
  {
    ...
    {
      Eigen::Vector3i query_id(id(0), id(1), z);
      if (md_.occupancy_buffer_inflate_[toAddress(query_id)])
      {
        return 1;
      }
    }
